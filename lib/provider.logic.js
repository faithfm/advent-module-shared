...
const core = {
  ...
  async fetchMember(provider_code, mobile){
    ...
    if (provider == null){
      log.warn({ evt: 'provider.not_found', providerCode: provider_code }, 'Provider not found');
      return null;
    }
  ...
  async addMember(code, mobile, role){
    ...
    if (provider == null){
      log.warn({ evt: 'provider.not_found', providerCode: code }, 'Provider not found');
      return null;
    }
  ...
  async createProvider(args){
    ...
    if (provider != null){
      log.info({ evt: 'provider.exists', providerCode: code }, 'Provider already exists');
    } else {
      var args = {
        code,
        name,
