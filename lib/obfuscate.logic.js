...
// Contact: for fields that hold a mobile OR an email depending on the channel
// (`to`/`from`). email() is idempotent by construction; mobile() self-inverse.
function contact(value) {
  if (value == null) return value;
  return String(value).indexOf('@') >= 0 ? email(value) : mobile(value);
}

module.exports = { mobile, email, address, name, contact, REDACTED };
