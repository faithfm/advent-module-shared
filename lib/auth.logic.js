...
const core = {
    ...
    async verifyToken(token) {
        ...
        if (token == null || token === undefined) {
            log.warn({ evt: 'auth.token_missing' }, 'Token is null or undefined');
            return {
                status: 401,
        ...
        if (token === process.env.API_MACHINE_TRUSTED_TOKEN) {
            log.debug({ evt: 'auth.trusted_service' }, 'Trusted request from another service');
            return {
                status: 200,
        ...
        try {
            ...
            log.error({ err: error, evt: 'auth.verify_failed' }, 'Error sending request to Auth service');
            return {
                status: 401,
